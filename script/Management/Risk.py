    ...
    def get_open_quantity(self, sec_id: str, is_buy: bool, open_price: float, stop_price: float):
        ...
        if sec_id in self.pos_mgr.positions:
            ...
            if open_qty * factor < 0:
                return 0
        (total_asset, free_cash, realized, unrealized) = self.pos_mgr.get_status()
        if free_cash / total_asset < (1 - self.capital_cap):
            return 0
        total_risk: float = self.pct * total_asset
        qty: int = int(total_risk/risk_per_share)
        qty = abs(qty) - abs(open_qty)
        if qty <= 0:
            return 0
        cost = ((qty * open_price) * (1 + sec.trans_cost_multiplier) + sec.trans_cost_fix_amount) \
            * sec.fx_rate * sec.conversion()
        if self.pos_mgr.support_margin:
            cost = ((qty * open_price) * (sec.init_margin_ratio + sec.trans_cost_multiplier)+sec.trans_cost_fix_amount)\
                   * sec.fx_rate * sec.conversion()
        if free_cash < cost:
            return 0
        return factor * qty
